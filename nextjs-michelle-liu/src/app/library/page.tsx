...
const SHELF_BOOKS_QUERY = `
  *[_type == "shelfItem" && isPublished == true && mediaType == "book"] | order(year desc, order asc) {
    _id,
    title,
    author,
    cover,
...
// Query to get all unique years from books
const BOOK_YEARS_QUERY = `
...
// BookDetailModal Component - simplified to show only available data
          ...
          <div className="basis-0 content-stretch flex flex-col gap-[24px] sm:gap-[28px] md:gap-[32px] lg:gap-[36px] grow items-start min-h-px min-w-px relative shrink-0 w-full sm:w-auto">
            <div className="content-stretch flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal gap-[4px] items-start leading-[0] relative shrink-0 text-[24px] sm:text-[26px] md:text-[28px] tracking-[0.38px] max-w-full">
              <div style={{ fontVariationSettings: "'wdth' 100" }} className="flex flex-col justify-center relative shrink-0 text-black">
                <p className="leading-[34px]">{book.title}</p>
              </div>
            ...
            <div className="content-stretch flex flex-col gap-[12px] sm:gap-[14px] md:gap-[16px] items-start relative shrink-0 w-full">
              {book.rating > 0 && (
                <div className="content-stretch flex flex-col sm:flex-row gap-[8px] sm:gap-[24px] md:gap-[32px] lg:gap-[40px] items-start sm:items-center leading-[0] relative shrink-0 text-nowrap w-full">
                  ...
                  <p className="font-['DM_Sans:Medium','Noto_Sans_Symbols2:Regular',sans-serif] leading-[1.4] not-italic relative shrink-0 text-gray-500 text-[18px] sm:text-[19px] md:text-[20px] text-center tracking-[0.6px]">
              ...
              {book.year && (
                <div className="content-stretch flex flex-col sm:flex-row gap-[8px] sm:gap-[24px] md:gap-[32px] lg:gap-[40px] items-start sm:items-center relative shrink-0 w-full">
                  <div style={{ fontVariationSettings: "'wdth' 100" }} className="flex flex-col font-['SF_Pro:Medium',sans-serif] font-[510] justify-center relative shrink-0 text-[17px] text-[rgba(0,0,0,0.4)] tracking-[-0.43px] leading-[0]">
                    <p className="leading-[22px]">Year Read</p>
                  </div>
                  <div className="bg-gray-100 content-stretch flex items-center justify-center px-[12px] py-[4px] relative rounded-[99px] shrink-0">
                    <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-gray-800 text-[15px] sm:text-[16px] text-nowrap tracking-[-0.31px]" style={{ fontVariationSettings: "'wdth' 100" }}>
                      <p className="leading-[21px]">{book.year}</p>
                    </div>
                  </div>
                </div>
              )}
...
export default function LibraryPage() {
  ...
  // Filter books by year (or show all)
          ...
          <div className="flex flex-col gap-[8px] items-start shrink-0">
            ...
            <div className="content-stretch flex gap-[12px] items-center relative shrink-0">
              ...
              <div className="relative">
                ...
                {showFilterDropdown && (
                  <div className="absolute left-0 top-[calc(100%+4px)] bg-white rounded-[8px] shrink-0 z-50 animate-in fade-in slide-in-from-top-1 duration-200 min-w-full shadow-lg">
                    <div aria-hidden="true" className="absolute border border-gray-100 border-solid inset-0 pointer-events-none rounded-[8px]" />
                    <div className="content-stretch flex flex-col items-start px-[8px] py-[6px] relative shrink-0 w-full max-h-[300px] overflow-y-auto">
                      {/* All option */}
                      <button
                        onClick={() => {
                          setActiveFilter("all");
                      ...
                      >
                        <div className={`relative rounded-[99px] shrink-0 w-full transition-colors ${
                          activeFilter === "all" ? 'bg-gray-200' : 'bg-gray-100 hover:bg-gray-200'
                        }`}>
                          <div className="flex flex-row items-center size-full">
                            <div className="content-stretch flex items-center px-[16px] py-[6px] relative w-full">
                              <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-gray-600 text-[20px] text-nowrap tracking-[-0.45px]" style={{ fontVariationSettings: "'wdth' 100" }}>
                                <p className="leading-[25px]">all</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {years.map((year) => (
                        ...
                        >
                          <div className={`relative rounded-[99px] shrink-0 w-full transition-colors ${
                            activeFilter === year ? 'bg-gray-200' : 'bg-gray-100 hover:bg-gray-200'
                          }`}>
                            <div className="flex flex-row items-center size-full">
                              <div className="content-stretch flex items-center px-[16px] py-[6px] relative w-full">
                                <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-gray-400 text-[20px] text-nowrap tracking-[-0.45px]" style={{ fontVariationSettings: "'wdth' 100" }}>
                                  <p className="leading-[25px]">{year}</p>
                                </div>
                              </div>
                            </div>
                          </div>
